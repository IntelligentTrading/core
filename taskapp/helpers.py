...
## migrate to smaller celery tasks

def get_source_trading_pairs(back_in_time_seconds=2*60*60):
    """
    ...
    return [(item['source'], item['transaction_currency'], item['counter_currency']) for item in price_objects]
...
def _compute_and_save_ann(source, resample_period=SHORT):
    if RUN_ANN:
        # choose the pre-trained ANN model depending on period, here are the same
        period2model = {
            SHORT : 'lstm_model_2_2.h5',
