...
from apps.channel import tasks
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # schedule.every().minute.do(_pull_poloniex_data) # old call (blocking)
        schedule.every().minute.do(
            tasks.pull_poloniex_data.delay # new. call via celery job queue
        ) 
        ...
        for horizont_period in PERIODS_LIST:
            ...
            if horizont_period in [SHORT, MEDIUM]:
                schedule.every(hours).hours.at("00:00").do(
#                    _compute_and_save_indicators,
                    tasks.compute_and_save_indicators,
                    {'period': horizont_period}
                )
